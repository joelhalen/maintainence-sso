...
MegaMTX is a full-stack maintenance management system built to replace costly third-party CMMS platforms (MaintainX or equivalent). The system enables maintenance teams to create, track, assign, and close work orders (called **tickets**) across a structured hierarchy of physical locations and assets. It is designed from the ground up to meet FDA 21 CFR Part 11 requirements for electronic records and electronic signatures, and now supports both self-hosted deployments and a managed cloud model with organization-scoped subscriptions.
...
- **Reduce wasteful CMMS spend** by paying only for the MegaMTX deployment model and plan that fits the organization.
- **Own or control the data path** — self-hosted deployments keep records in a customer-managed PostgreSQL database, while managed cloud deployments isolate each organization's records and preserve exportability.
...
- **Cost control**: Choose self-hosted infrastructure or a managed cloud subscription with plan limits that match actual usage.
...
- **Data ownership and portability**: Data remains tenant-scoped and exportable whether the deployment is self-hosted or managed by MegaMTX.
...
### Tenancy and Subscription Boundary

Managed cloud deployments use an `Organization` as the tenant boundary. Authenticated requests hydrate `req.user.organizationId` and an organization subscription context; every core query and mutation must include that tenant key. RBAC answers whether the current user has permission to perform an action, while subscription entitlements answer whether the organization is allowed to use the feature or create more capped resources.

Current entitlement checks are enforced in the Express API before creating active users, locations, assets, or active tickets. Plan flags also gate SMS, SSO, and exports. PayPal metadata is stored on `OrganizationSubscription` for future checkout and webhook integration, but billing side effects are not part of the current runtime flow.
...
| **Twilio** | — | SMS delivery for ticket notifications and future verification / 2FA flows. All sends are logged in PostgreSQL before calling the provider. |
...
**Purpose:** `Organization` is the tenant boundary for cloud-hosted MegaMTX. Users, roles, locations, assets, tickets, comments, attachments, signatures, audit logs, and notification logs are scoped to an organization. `SubscriptionPlan` defines tier limits and feature flags, and `OrganizationSubscription` records the organization's current plan, status, and PayPal-ready provider identifiers.

Seeded tiers are `FREE`, `STARTER`, `PROFESSIONAL`, and `ENTERPRISE`. Nullable numeric limits mean unlimited. Backend entitlement checks treat `ACTIVE` and `TRIALING` subscriptions as usable and block capped creates when usage reaches the plan limit.
...
| `organizationId` | String | FK → Organization, not null | Tenant boundary for auth and data access |
| `department` | String? | Nullable | Organizational department |
| `phone` | String? | Nullable | E.164 phone number for SMS notifications and verification |
| `phoneVerifiedAt` | DateTime? | Nullable | Set when a phone verification flow confirms ownership |
...
**Indexes:** `email` (for login lookups), `roleId` (for permission queries), `organizationId` (for tenant-scoped queries).
...
|---|---|---|---|
| `id` | String (cuid) | PK | |
| `organizationId` | String | FK → Organization, not null | Roles are scoped to a tenant |
| `name` | String | Unique per organization, not null | e.g., `Super Admin`, `Technician` |
...
|---|---|---|---|
| `id` | String (cuid) | PK | |
| `organizationId` | String | FK → Organization, not null | Tenant boundary |
| `name` | String | Not null | Human-readable name |
| `code` | String? | Unique per organization, nullable | Short reference code, e.g., `MAIN-B1-F2` |
...
**Relationships:** `location`, `asset`, `createdBy`, `assignedTo`, `completedBy`, `closedBy`, `comments` (TicketComment[]), `attachments` (Attachment[]), `statusHistory` (TicketStatusHistory[]), `emailLogs` (EmailLog[]), `smsLogs` (SmsLog[]), `signatures` (ElectronicSignature[]).
...
**Purpose:** Stores normalized inbound and outbound email content for the super-admin mailbox view and ticket reply threading. Outbound `EmailMessage` records are created after SMTP accepts a send; inbound records are created by IMAP polling or future provider webhooks.

| Field | Type | Constraints | Notes |
|---|---|---|---|
| `id` | String (cuid) | PK | |
| `organizationId` | String | FK → Organization | Tenant scope |
| `direction` | EmailDirection enum | Not null | INBOUND or OUTBOUND |
| `provider` | String | Not null | smtp, imap, mailgun, postmark, etc. |
| `messageId` | String? | Unique per organization | RFC Message-ID when available |
| `inReplyTo` | String? | Nullable | Header used for reply matching |
| `references` | String[] | Default [] | Header chain used for reply matching |
| `from` | String | Not null | Sender address |
| `to` / `cc` | String[] | Not null | Recipients |
| `subject` | String | Not null | |
| `textBody` / `htmlBody` | String? | Nullable | Plain text preferred for ticket comments |
| `status` | EmailMessageStatus enum | Default RECEIVED | RECEIVED, PROCESSED, IGNORED, FAILED |
| `ticketId` | String? | FK → Ticket, nullable | Matched ticket if threading succeeds |
| `userId` | String? | FK → User, nullable | Sender user or system inbound-email user |
| `receivedAt` / `sentAt` / `processedAt` | DateTime? | Nullable | Lifecycle timestamps |

### EmailAttachment

**Purpose:** Stores metadata and filesystem paths for inbound email attachments. Files are saved under the configured upload directory in an `email-attachments` subdirectory.
...
| `filename` | String | Not null | Stored UUID-based filename |
| `originalName` | String | Not null | Original attachment filename |
| `contentType` | String | Not null | MIME type from parser |
| `sizeBytes` | Int | Not null | |
...
Super admins with `EMAIL_SETTINGS` can access `/settings/email` to inspect SMTP/IMAP configuration status, verify connections, send a test email, review outbound `EmailLog` records, poll the inbox manually, and view stored `EmailMessage` records. Secrets remain in environment variables; the API only reports whether sensitive values are configured.

### Inbound Email Architecture

Inbound email initially uses IMAP polling for hobby/self-hosted deployments. The server reads `IMAP_*` environment variables, opens the configured mailbox, parses recent messages with `mailparser`, stores normalized metadata in `EmailMessage`, and stores attachment metadata in `EmailAttachment`.

Ticket reply threading uses these matching rules:
1. Prefer plus-address tokens such as `maintenance+ticket-MNT-2026-00047@example.com`.
2. Fall back to `In-Reply-To` and `References` headers matching stored outbound `EmailMessage.messageId` values.
3. Fall back to a subject token such as `[MNT-2026-00047]`.
...
The managed cloud topology runs the same Express API and React app, but treats `Organization` as the isolation boundary inside a shared application plane. All tenant-owned tables include `organizationId`; API handlers must filter by the authenticated organization and validate that referenced users, locations, assets, and tickets belong to the same tenant.

Cloud billing is modeled through `SubscriptionPlan` and `OrganizationSubscription`. PayPal is the intended payment provider, with provider IDs stored in the database for future checkout and webhook handling. Until that integration ships, plan assignment is an administrative operation and entitlement checks are enforced entirely inside the API.
...
| `EMAIL_REPLY_DOMAIN` | Domain used for ticket reply-to addresses | `company.com` |
| `EMAIL_REPLY_LOCAL_PART` | Local part used for plus-address ticket replies | `maintenance` |
| `IMAP_HOST` | Inbound mailbox IMAP host | `imap.company.com` |
| `IMAP_PORT` | Inbound mailbox IMAP port | `993` |
| `IMAP_USER` | Inbound mailbox username | `maintenance@company.com` |
| `IMAP_PASS` | Inbound mailbox app password | _(secret)_ |
| `IMAP_MAILBOX` | Mailbox folder to poll | `INBOX` |
| `IMAP_POLLING_ENABLED` | Enables background IMAP polling | `true` or `false` |
...
| `PAYPAL_ENVIRONMENT` | PayPal API environment | `sandbox` or `live` |
...
- Organization tenancy with subscription plans, usage limits, and PayPal-ready billing metadata
...
- **Multi-site support inside organizations**: Site-scoped access control within a tenant, allowing one organization to delegate access across plants while preserving organization-level subscription and audit boundaries.
- **PayPal billing integration**: Checkout sessions, subscription webhooks, grace periods, dunning states, and self-service plan changes wired to the existing subscription models.
